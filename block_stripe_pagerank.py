...
def load_data():
    ...
    for line in link_matrix:
        for dest in line[2]:
            ...
            link_matrix_groups[group_id][line[0]][1]+=1
...
def compute_rnew():
    ...
    for i in range(group_num):
        ...
        r_new_stripe =np.ones(get_group_size())*(1-belta) / Num
        #读取r_old_stripe
        with open(R_VECTOR_PATH_PREFIX+str(i)+R_VECTOR_PATH_SUFFIX,'rb') as f:
            r_old_stripe=pkl.load(f)
        with open(LINK_MATRIX_PATH_PREFIX+str(i)+LINK_MATRIX_PATH_SUFFIX,'rb') as f:
            while True:
                try:
                    # 逐行读取
                    row_data = pkl.load(f)
...
def block_stripe_pagerank():
    ...
    #分块 储存
    for i in range(group_num):
        r_stripe =np.ones(get_group_size()) / get_group_size()
        with open(R_VECTOR_PATH_PREFIX+str(i)+R_VECTOR_PATH_SUFFIX,'wb') as f:
            pkl.dump(r_stripe,f)
    #compute the r_new
    while True:
...
def print_result():
    ...
    for i in range(group_num):
        ...
        for j in range(OUTPUT_NUM):
            results.append((index[j],r_stripe[index[j]]))
    #top100排序
